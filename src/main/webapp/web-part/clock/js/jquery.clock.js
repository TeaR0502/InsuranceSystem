...
(function($) {
	$.fn.hoverclock = function(options) {
		...
		var settings = $.extend({
			...
			h_width : "300px",// 宽度
			h_height : "300px",// 高度
			h_secondHandColor : "#F08080",// 秒针演示
			h_frontColor : "#66CDAA",
			h_thinHandColor : "#66CDAA",// 分针颜色
		...
		h_hoverClock
				.append('<div class="smallhovertree"></div><div class="bighovertree"></div><div class="secondhovertree"></div><div class="secondhovertree2"></div><div class="minutehovertree"></div><div class="hourhovertree"></div>	');
		h_hoverClock.css({
			"width" : settings.h_width,
			"height" : settings.h_height,
			"background-color" : settings.h_backColor,
			"border-color" : settings.h_borderColor
		});

		var h_hoverSecond = h_hoverClock.find(".secondhovertree"), h_hoverBig = h_hoverClock
				.find(".bighovertree"), h_hoverSmall = h_hoverClock
				.find(".smallhovertree"), h_hoverMinute = h_hoverClock
				.find(".minutehovertree"), h_hoverHour = h_hoverClock
				.find(".hourhovertree");
		h_hoverSecond2 = h_hoverClock.find(".secondhovertree2");
		var h_minBanjing = Math
				.min(h_hoverClock.width(), h_hoverClock.height()) / 2;
		...
		setInterval(
				function() {
					...
					document.querySelector(".secondhovertree").style.transform = "rotate("
							+ h_secondRadian + "deg)";
					h_hoverMinute.css({
						"transform" : " rotate(" + h_minuteRadian + "deg)"
					});
					h_hoverHour.css({
						"transform" : " rotate(" + h_hourRadian + "deg)"
					});
					h_hoverSecond2.css({
						"transform" : " rotate(" + (h_secondRadian + 180)
								+ "deg)"
					});
				}, 50);
		...
		// 设置圆心大小圆位置
		h_hoverBig.css({
			"left" : (h_hoverClock.width() - h_hoverBig.width()) / 2,
			"top" : (h_hoverClock.height() - h_hoverBig.height()) / 2
		});
		h_hoverSmall.css({
			"left" : (h_hoverClock.width() - h_hoverSmall.width()) / 2,
			"top" : (h_hoverClock.height() - h_hoverSmall.height()) / 2
		});

		// 设置秒针位置
		h_hoverSecond.css({
			"left" : (h_hoverClock.width() - h_hoverSecond.width()) / 2,
			"top" : (h_hoverClock.height() / 2 - h_hoverSecond.height()),
			"background-color" : settings.h_secondHandColor
		});
		h_hoverSecond2.css({
			"left" : (h_hoverClock.width() - h_hoverSecond.width()) / 2,
			"top" : (h_hoverClock.height() / 2 - h_hoverSecond2.height()),
			"background-color" : settings.h_secondHandColor
		});
		// 设置分针位置，边框圆角
		h_hoverMinute.css({
			"left" : (h_hoverClock.width() - h_hoverMinute.width()) / 2,
			"top" : (h_hoverClock.height() / 2 - h_hoverMinute.height()),
			"border-radius" : h_hoverMinute.width() / 2,
			"background-color" : settings.h_thinHandColor
		});
		// 设置时针位置，边框圆角
		h_hoverHour.css({
			"left" : (h_hoverClock.width() - h_hoverHour.width()) / 2,
			"top" : (h_hoverClock.height() / 2 - h_hoverHour.height()),
			...
			h_scale.appendTo(h_hoverClock).css(
					{
						...
						"top" : (h_hoverClock.height() / 2 - h_scale.height()),
						"transform" : "rotate(" + deg + "deg) translateY("
								+ (h_scale.height() - radii) + "px)"
					});
		}
			...
			if (text != "") {
				h_scale
						.html('<span style="color:'
								...
								+ textsize
								+ 'px;position:absolute;bottom:0px;font-weight:bold;text-align:center;width:100%;transform:rotate(-'
								+ deg + 'deg)">' + text + "<span>");
		...
		if (settings.h_hourNumShow) {
			...
			for (var i = 0; i < 12; i++) {
				...
				if (h_hourNum == 0)
					h_hourNum = 12;
				createNum(h_deg, 90, "transparent", h_hourNum,
						settings.h_hourNumColor, settings.h_hourNumSize,
						settings.h_hourNumRadii);
			}
		}
		...
		for (var i = 0; i < 60; i++) {
			...
			if ((i % 5) == 0) {
				...
				if (settings.h_minuteNumShow) {
					createNum(h_hudu, 80, "transparent", h_text, "green",
							settings.h_minute5Height + 4, h_scaleRadii - 4);
				} else {
					if ((i % 15) == 0) {

						createsSaleHovertree(settings.h_minute15Width,
								settings.h_minute15Height, h_hudu,
								settings.h_frontColor, h_scaleRadii);
					} else {
						createsSaleHovertree(settings.h_minute5Width,
								settings.h_minute5Height, h_hudu,
								settings.h_frontColor, h_scaleRadii);
					}
				}
			} else {
				createsSaleHovertree(settings.h_minuteWidth,
						settings.h_minuteHeight, h_hudu, settings.h_frontColor,
						h_scaleRadii);
			}
		...
		// ---------------------------------------------------
		h_hoverClock
				.append('<div class="hoverclocktext"><a href="http://hovertree.com/texiao/hoverclock/" target="_blank" class="hoverclocklink" style="text-decoration:none">HoverClock</a></div>');
		h_hoverClock.find(".hoverclocktext").css({
			"width" : h_hoverClock.width(),
			"position" : "absolute",
			"top" : h_minBanjing * 6 / 5,
			"text-align" : "center"
		});
		...
		h_hoverClockLink.css({
			"color" : settings.h_linkColor,
			"font-size" : settings.h_linkSize
		});
		// ----------------------------------------------------
