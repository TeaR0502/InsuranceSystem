...
(function() {
	...
	if (window.WEB_SOCKET_LOGGER) {
		...
		// In some environment, console is defined but console.log or
		// console.error is missing.
	...
	if (location.protocol == "file:") {
		logger.error("WARNING: web-socket-js doesn't work in file:///... URL "
	...
	window.WebSocket = function(url, protocols, proxyHost, proxyPort, headers) {
		...
		self.__createTask = setTimeout(function() {
			WebSocket.__addTask(function() {
				...
				WebSocket.__flash.create(self.__id, url, protocols, proxyHost
	...
	WebSocket.prototype.send = function(data) {
		...
		// Note by wtritch: Hopefully this will not be necessary using
		// ExternalInterface. Will require
		// additional testing.
		var result = WebSocket.__flash
				.send(this.__id, encodeURIComponent(data));
		if (result < 0) { // success
			return true;
		} else {
	...
	WebSocket.prototype.__createMessageEvent = function(type, data) {
		if (document.createEvent && window.MessageEvent && !window.opera) {
			...
			// IE and Opera, the latter one truncates the data parameter after
			// any 0x00 bytes.
			return {
				type : type,
				data : data,
	...
	WebSocket.__initialize = function() {
		...
		if (!window.WEB_SOCKET_SUPPRESS_CROSS_DOMAIN_SWF_ERROR
			...
			if (location.host != swfHost) {
				logger
						.error("[WebSocket] You must host HTML and WebSocketMain.swf in the same host "
								+ "('"
								+ location.host
								+ "' != '"
								+ swfHost
	...
	/**
	 * Called by Flash to notify JS that it's fully loaded and ready for
	 * communication.
	 */
	...
	WebSocket.__onFlashEvent = function() {
		setTimeout(function() {
			try {
				// Gets events using receiveEvents() instead of getting it from
				// event object
				// of Flash event. This is to make sure to keep message order.
				// It seems sometimes Flash events don't arrive in the same
				// order as they are sent.
				var events = WebSocket.__flash.receiveEvents();
				for (var i = 0; i < events.length; ++i) {
					WebSocket.__instances[events[i].webSocketId]
